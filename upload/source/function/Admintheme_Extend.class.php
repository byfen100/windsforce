...
class Admintheme_Extend{
	...
	static public function path($sFile,$sType='images',$bReturnurl=true){
		...
		if(is_file(WINDSFORCE_PATH.'/admin/Theme/'.$sAdmintemplate.'/Public/'.$sType.'/'.$sFile)){
			if($bReturnurl===true){
				...
				return WINDSFORCE_PATH.'/admin/Theme/'.$sAdmintemplate.'/Public/'.$sType.'/'.$sFile;
			}
		}else{
			if($bReturnurl===true){
				return __TMPLPUB__DEFAULT__.'/'.$sType.'/'.$sFile;
			}else{
				return WINDSFORCE_PATH.'/admin/Theme/Default/Public/'.$sType.'/'.$sFile;
			}
		}
	}
	...
	static public function cssCompress($sCssfile,$sResourcefile=''){
		...
		$sCssData=str_replace('../Images/',__ROOT__.'/admin/Theme/'.$sTheme.'/Public/Images/',$sCssData);

		return $sCssData;
	}

	static public function cssPath($sFile,$sResourcefile=''){
		$sExtName=G::getExtName($sFile,2);
