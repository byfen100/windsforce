...
/* [$DoYouHaoBaby] (C)WindsForce TEAM Since 2010.10.04.
   DoYouHaoBaby 框架基础文件($Liu.XiangMin)*/
/** 防止乱码 */
...
if(!defined('DOYOUHAOBABY_DEBUG')){
	define('DOYOUHAOBABY_DEBUG',FALSE);
}
if(DOYOUHAOBABY_DEBUG===TRUE){
...
/* [$DoYouHaoBaby] (C)WindsForce TEAM Since 2010.10.04.
   数据库工厂类，用于生成数据库相关对象 < 抽象类 >($Liu.XiangMin)*/
...
class ModelBehaviorRbac extends ModelBehavior{
	...
	public function checkRbac(){
		...
		//G::dump(Dyhb::normalize($GLOBALS['_commonConfig_']['NOT_AUTH_MODULE']));
...
//G::dump((!empty($arrAction['yes']) and in_array(strtoupper(ACTION_NAME),$arrAction['yes'])));
				//G::dump((!empty($arrAction['no']) and !in_array(strtoupper(ACTION_NAME),$arrAction['no'])) ||
			//		(!empty($arrAction['yes']) and in_array(strtoupper(ACTION_NAME),$arrAction['yes'])));
				...
				//	echo 'xxx';
					if($GLOBALS['_commonConfig_']['RBAC_GUEST_ACCESS'] && is_array($GLOBALS['_commonConfig_']['RBAC_GUEST_ACCESS'])){
						//G::dump(in_array(APP_NAME.'@*@*',$GLOBALS['_commonConfig_']['RBAC_GUEST_ACCESS']));
						//G::dump(APP_NAME.'@*@*');
						//G::dump($GLOBALS['_commonConfig_']['RBAC_GUEST_ACCESS']);
...
/* [$DoYouHaoBaby] (C)WindsForce TEAM Since 2010.10.04.
   ModelRelationHasMany类封装了对象见的一对多关系($Liu.XiangMin)*/
...
/* [$DoYouHaoBaby] (C)WindsForce TEAM Since 2010.10.04.
   ModelRelationColl 封装了对象的关联关系，并且提供操作这些关联关系的方法($Liu.XiangMin)*/
