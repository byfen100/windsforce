...
/** DoYouHaoBaby框架定义 | 本版本于2013/05/22发布 */
define('DYHB_VERSION','2.5');
/** 定义内存 */
	...
	static public function U($sUrl,$arrParams=array(),$bNormalurl=false,$bRedirect=false,$bSuffix=true){
		...
		if($GLOBALS['_commonConfig_']['URL_MODEL']>0 && $bNormalurl===false){
			...
			if(!empty($sRoute)){
				...
				$sStr=substr($sStr,0,-1);
				$sUrl=(__APP__!=='/'?__APP__:'').($GLOBALS['_commonConfig_']['DEFAULT_APP']!=$sApp?'/app'.$sDepr.$sApp.$sDepr:'/').$sModule.$sDepr.$sAction.$sStr;
			}
...
class ModelBehaviorRbac extends ModelBehavior{
	...
	public function replaceSession($sHash,$nUserId,$sAuthKey,$bInsert=false){
		...
		$sIp=G::getIp();
		$oSession=SessionModel::M()->deleteWhere("`session_hash`='{$sHash}' OR($nUserId<>0 AND `user_id`={$nUserId})");// 删除SESSION
