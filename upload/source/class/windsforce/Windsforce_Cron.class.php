...
/* [$WindsForce] (C)WindsForce TEAM Since 2012.03.17.
   计划任务执行类 && Modify From Discuz!($Liu.XiangMin)*/

!defined('DYHB_PATH') && exit;

/** 导入进程类 */
if(!Dyhb::classExists('Windsforce_Process')){
...
class Windsforce_Cron{
	...
	public static function RUN($nCronid=0){
		$oCron=$nCronid?CronModel::F('cron_id=? AND cron_status=1',$nCronid)->getOne():Dyhb::instance('CronModel')->fetchNextrun(CURRENT_TIMESTAMP);
		...
		if(!empty($oCron['cron_id'])){
			...
			if(count($arrFile)>1){
				if($oCron['cron_type']=='app'){
					$sCronfile=in_array($arrFile[0],$GLOBALS['_cache_']['app'])?WINDSFORCE_PATH.'/app/'.$arrFile[0].'/App/Class/Extension/cron/'.$arrFile[1]:'';
				}elseif($oCron['cron_type']=='plugin'){
					$sCronfile=in_array($arrFile[0],array('helloworld'))?WINDSFORCE_PATH.'/ucontent/plugin/'.$arrFile[0].'/cron/'.$arrFile[1]:'';
				}
			}else{
				if($oCron['cron_type']=='user'){
					$sCronfile=WINDSFORCE_PATH.'/ucontent/cron/'.$oCron['cron_filename'];
				}else{
					$sCronfile=WINDSFORCE_PATH.'/source/cron/'.$oCron['cron_filename'];
				}
			}
	...
	private static function nextcron(){
		$oCron=Dyhb::instance('CronModel')->fetchNextcron();
	...
	private static function setNextime($oCron){
		...
		$oUpdateCron->save(0,'update');

		if($oUpdateCron->isError()){
			$this->E($oUpdateCron->getErrorMessage());
		}
