...
CREATE TABLE `{WINDSFORCE}event` (
  ...
  `event_cover` int(10) NOT NULL COMMENT '活动封面',
...
CREATE TABLE `{WINDSFORCE}eventcomment` (
  ...
  `eventcomment_content` text NOT NULL COMMENT '内容',
  `eventcomment_email` varchar(300) NOT NULL COMMENT '邮件',
  `eventcomment_url` varchar(300) NOT NULL COMMENT 'URL',
  `eventcomment_status` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否显示',
  `eventcomment_ip` varchar(16) NOT NULL COMMENT 'IP',
  `eventcomment_parentid` int(10) NOT NULL DEFAULT '0' COMMENT '父级ID',
  `eventcomment_isreplymail` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否邮件通知，通知给留言者',
  `eventcomment_ismobile` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为手机评论',
  `eventcomment_auditpass` tinyint(1) NOT NULL DEFAULT '1' COMMENT '审核是否通过',
  `event_id` int(10) NOT NULL DEFAULT '0' COMMENT '活动ID',
  PRIMARY KEY (`eventcomment_id`),
  KEY `user_id` (`user_id`),
  KEY `create_dateline` (`create_dateline`),
  KEY `eventcomment_status` (`eventcomment_status`),
  KEY `eventcomment_userid_id` (`event_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8;
