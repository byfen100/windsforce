	...
	static public function checkGroup($oGroup,$bCheckAdd=false,$nUserid=null){
		...
		if(!is_object($oGroup)){
			...
			if(empty($oGroup['group_id'])){
				Dyhb::E(Dyhb::L('小组不存在或者还在审核中','Controller'));
			}
		}

		if($oGroup->group_isopen==0){
			if($GLOBALS['___login___']===false){
				G::urlGoTo(Core_Extend::windsforceReferer(),3,Dyhb::L('只有该小组成员才能够访问小组','Controller').'&nbsp;&nbsp;'.Dyhb::L('首先你需要的登录后才能够继续操作','Controller'));
			}
			
			$oGroupuser=GroupuserModel::F('user_id=? AND group_id=?',$nUserid,$oGroup['group_id'])->getOne();
			if(empty($oGroupuser['user_id'])){
				Dyhb::E(Dyhb::L('只有该小组成员才能够访问小组','Controller').'&nbsp;<span id="listgroup_'.$oGroup['group_id'].'" class="commonjoinleave_group"><a href="javascript:void(0);" onclick="joinGroup('.$oGroup['group_id'].',\'listgroup_'.$oGroup['group_id'].'\');">'.Dyhb::L('我要加入','Controller').'</a></span>');
			}
		}
		...
		if($bCheckAdd===true){
			if($oGroup->group_ispost==0){
				...
				if(empty($oGroupuser['user_id'])){
					Dyhb::E(Dyhb::L('只有该小组成员才能发帖','Controller').'&nbsp;<span id="listgroup_'.$oGroup['group_id'].'" class="commonjoinleave_group"><a href="javascript:void(0);" onclick="joinGroup('.$oGroup['group_id'].',\'listgroup_'.$oGroup['group_id'].'\');">'.Dyhb::L('我要加入','Controller').'</a></span>');
				}
			}elseif($oGroup->group_ispost==1){
				Dyhb::E(Dyhb::L('该小组目前拒绝任何人发帖','Controller'));
			}
		}
