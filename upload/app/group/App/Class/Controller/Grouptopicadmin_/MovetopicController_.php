...
class MovetopicController extends Controller{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkTopicmove()){
			$this->E(Dyhb::L('你没有移动帖子的权限','Controller'));
		}

		if(empty($sMovegroupid)){
			$this->E(Dyhb::L('你没有选择需要移动到的目标小组','Controller'));
		}
		...
		if(empty($oGroup['group_id'])){
			$this->E(Dyhb::L('你需要移动到的目标小组不存在','Controller'));
		}
			...
			foreach($arrGrouptopics as $nGrouptopic){
				...
				if(!empty($oGrouptopic['grouptopic_id']) && $oGrouptopic->group_id!=$oGroup->group_id){
					...
					if($GLOBALS['___login___']['user_id']!=$oGrouptopic['user_id']){
						$sNoticetemplate='<div class="notice_movegrouptopic"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.Dyhb::L('对你的主题执行了移动','Controller').'&nbsp;<a href="{@grouptopic_link}">'.$oGrouptopic['grouptopic_title'].'</a></span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Dyhb::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopic_link}">'.Dyhb::L('查看','Controller').'</a></div></div>';
		...
		$this->A(array('group_id'=>$nGroupid,array('move_groupid'=>$oGroup['group_id'])),Dyhb::L('移动主题成功','Controller'));
	}
