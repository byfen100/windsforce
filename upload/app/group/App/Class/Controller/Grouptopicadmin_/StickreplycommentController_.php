...
class StickreplycommentController extends Controller{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkCommentadminRbac($oGrouptopic->group,array('group@grouptopicadmin@stickreplycomment'))){
			$this->E(Dyhb::L('你没有权限置顶或者取消置顶回帖','Controller'));
		}
			...
			foreach($arrGrouptopiccomments as $nGrouptopiccomment){
				...
				if(!empty($oGrouptopiccomment['grouptopiccomment_id']) && $oGrouptopiccomment->grouptopiccomment_stickreply!=$nStatus){
					...
					if($GLOBALS['___login___']['user_id']!=$oGrouptopiccomment['user_id']){
						$sNoticetemplate='<div class="notice_'.($nStatus==1?'stickreplycomment':'unstickreplycomment').'"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.($nStatus==1?Dyhb::L('对你的回帖执行了置顶','Controller'):Dyhb::L('对你的回帖执行了取消置顶','Controller')).'</span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Dyhb::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopiccomment_link}">'.Dyhb::L('查看','Controller').'</a></div></div>';
		...
		$this->A(array('group_id'=>$nGroupid,'grouptopic_url'=>$sGrouptopicurl),$nStatus==1?Dyhb::L('置顶回帖成功','Controller'):Dyhb::L('取消置顶回帖成功','Controller'));
	}
