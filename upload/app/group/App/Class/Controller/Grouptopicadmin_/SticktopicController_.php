...
class SticktopicController extends Controller{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkTopicadminRbac($nGroupid,array('group@grouptopicadmin@sticktopic'))){
			$this->E(Dyhb::L('你没有置顶或者取消置顶帖子的权限','Controller'));
		}
			...
			foreach($arrGrouptopics as $nGrouptopic){
				...
				if(!empty($oGrouptopic['grouptopic_id']) && $oGrouptopic->grouptopic_sticktopic!=$nStatus){
					...
					if($GLOBALS['___login___']['user_id']!=$oGrouptopic['user_id']){
						$sNoticetemplate='<div class="notice_'.($nStatus==1?'stickgrouptopic':'unstickgrouptopic').'"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.($nStatus==1?Dyhb::L('对你的主题执行了置顶','Controller'):Dyhb::L('对你的主题执行了取消置顶','Controller')).'&nbsp;<a href="{@grouptopic_link}">'.$oGrouptopic['grouptopic_title'].'</a></span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Dyhb::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopic_link}">'.Dyhb::L('查看','Controller').'</a></div></div>';
		...
		$this->A(array('group_id'=>$nGroupid),$nStatus==0?Dyhb::L('取消置顶主题成功','Controller'):Dyhb::L('置顶主题成功','Controller'));
	}
