	 ...
	 */
	 ...
	 emotion.prototype = {
	    ...
	    insert: function(num) {
	        ...
	        if(text && this.input){
				if(getObjectClass(this.input)=='init'){
					$(this.input).insertText(text);
					$(this.input).focus();
					this.insertCallback && this.insertCallback(this.input);
				}else{
					this.input.insertHtml(text);
				}
			}
	    }
	 };
	...
	var wbSubjoin = (function() {
		return {
				...
				for (j = 0; j < emotionsClsArray.length; j++) {
					...
					$.each(emotionsCls[emotionsClsArray[j]], function(i, item) {
						...
						if (mtype == 'ani') {
							content += '<span>'
									+ item[0]
									+ '</span><span class="play_btn" ></span></a>';
							// 动漫表情内容分页
							(k % 14 == 0)
					...
					if (mtype == 'ani') {
						...
						if ((k - 1) / 14 > 1) {
							...
							for (i = 2; i <= ((k - 1) / 14 + 1); i++) {
								content += '<a href="#">' + i + '</a>';
							}
						}
						content += '</span>点击<img src="'+_ROOT_+'/Public/js/ffemotion/css/bgimg/emotion_play.gif" align="absmiddle">预览魔法动画</div>';
					}
					content += '</div>';
				}
				emotionsClsStr += "</div>";
			...
			createEmotion : function() {
				...
				// 魔法表情播放按钮
				$('.play_btn').live('click', function(e) {
					src = $(e.target).parent().find('img').attr('src').replace(
							'.gif', '.swf');
					This.showMagicEmotion(src);
					return false;
				});
